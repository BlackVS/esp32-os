...
#define GAME_X1 0+2
#define GAME_X2 160-1-2
#define GAME_HEIGHT (GAME_X2-GAME_X1+1)

#define GAME_Y1 0
#define GAME_Y2 80-1-2
...
class CCorona: public NANO_SPRITE_TYPE
{
public:
    CCorona()
    : NANO_SPRITE_TYPE({16, 8}, {8, 8}, nullptr) 
    { 
    }
    ...
    void draw() override
    {
        getTiler().getCanvas().setColor(RGB16_PINK);
...
class CFighter: public NANO_SPRITE_TYPE
{
public:
    CFighter()
    : NANO_SPRITE_TYPE({16, 8}, {8, 8}, fighterImage) 
    { 
    }
    ...
    void draw() override
    {
        getTiler().getCanvas().setColor(RGB16_GREEN);
        NANO_SPRITE_TYPE::draw();
    }
...
class CBullet: public NANO_SPRITE_TYPE
{
public:
    CBullet()
    : NANO_SPRITE_TYPE({16, 8}, {8, 8}, bulletImage) 
    { 
    }
    ...
    void draw() override
    {
        getTiler().getCanvas().setColor(RGB16_WHITE);
        NANO_SPRITE_TYPE::draw();
    }
...
bool onDraw()
{
    ...
    engine.getCanvas().setColor(RGB16_WHITE);

    //show score
    if(game_lives)
        engine.getCanvas().drawVLine(0,  h-1-(game_lives*h)/game_lives_max,h-1);

    if(game_total_score)
        engine.getCanvas().drawVLine(w-1,h-1-(game_total_score*h)/game_total_score_max,h-1);
...
{
    display.clear();
    display.setColor(RGB16_WHITE);
    display.drawBitmap1(16, 8, 128, 64, fighter_stop_corona);
    vTaskDelay(200);
    display.drawBitmap1(16, 8, 128, 64, fighter_masks);
    vTaskDelay(100);
    display.drawBitmap1(16, 8, 128, 64, fighter_stay_at_home);
    vTaskDelay(100);
    display.drawBitmap1(16, 8, 128, 64, fighter_or);
    vTaskDelay(100);
    display.drawBitmap1(16, 8, 128, 64, fighter_covid_die);
    vTaskDelay(100);
}
...
void setup_screen_saver_alive(){
  ...
  display.drawBitmap1(16, 8, 128, 64, fighter_masks);
  vTaskDelay(100);
  display.setColor(RGB16_GREEN);
  display.drawBitmap1(16, 8, 128, 64, fighter_covid_cancel);
  vTaskDelay(20);
}
